...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        # mapped_expr = expr.map(lambda expr : scheme_eval(expr,env))
        evaled_first = scheme_eval(first,env)

        check_procedure(evaled_first)
        if isinstance(evaled_first, MacroProcedure):
            return scheme_eval(evaled_first.apply_macro(rest,env), env)
        return scheme_apply(evaled_first,rest.map(lambda x: scheme_eval(x,env)),env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) < len(vals):
            raise SchemeError('Too many arguements given')
        if len(formals) > len(vals):
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Wrong number of arguments passed in {}'.format(self.fn))
        # END PROBLEM 3
...
def do_and_form(expressions, env):
    ...
    # if scheme_truep(expressions.first):
    #     return scheme_eval(expressions.first, env, True)
    # else:
    #     return False

    if len(expressions.rest) == 0:
        return scheme_eval(expressions.first, env, True)
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        actuals = Pair(scheme_eval(bindings.first.rest.first, env), actuals)

        bindings = bindings.rest

    check_formals(formals)
    return env.make_child_frame(formals,actuals)
    # END PROBLEM 14
